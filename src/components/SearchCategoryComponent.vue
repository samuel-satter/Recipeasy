      ...
      <form>
        <div>
          <input
            ...
            :placeholder="`Sök inom kategori ${this.$route.params.categoryId}`"
            name="search"
          />
    ...
    <div v-for="recipe in searchResult" :key="recipe.id">
      <div class="grid-container gradient-background main">
        <h2 class="recipe-head">
          ...
          <RatingComponent :avg-rating="`${recipe.ratings.length}`"> </RatingComponent>
        </h2>
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      fetch(
        `http://localhost:3000/categories/${this.$route.params.categoryId}/recipes/?query=${this.searchBoxInput}`
      )
